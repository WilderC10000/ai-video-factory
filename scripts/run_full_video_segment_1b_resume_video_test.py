#!/usr/bin/env python3
"""RESUME Segment 1B: video-only completion from the already-approved,
LOCKED start-frame image.

scripts/run_full_video_segment_1b_test.py's mandatory review gate worked
exactly as designed - it stopped after the $0.15 image call and before the
$0.40 video call. The user exited at that gate (accidentally), and the
resulting image (segment_1b_start_frame.jpg) was then reviewed separately
and approved/locked as the canonical Segment 1B starting image. This
...
the remaining Wan 3.0 480p 8-second video call from it.

The video prompt is imported directly from run_full_video_segment_1b_test
(VIDEO_PROMPT) - not retyped, and used completely UNCHANGED (no added
emphasis sentence this time, unlike segment 1A's resume) - so it is
byte-identical to the already-approved hard-time-jump prompt with its 4
distinct stages (footprint marked -> foundations/materials staged -> first
joists appear -> most joists installed) and the preserved "no floor
decking, no wall framing, no roof, and no windows or doors" S1 end-state
constraint. The video model config (WAN_3_0_SEGMENT_1B, a local duration=8
...
guaranteed identical to what was already approved.

Validates against the segment's own manifest.json (written by
run_full_video_segment_1b_test.py's image step, before the review gate) -
confirming its image_path matches the locked image, that the image was
actually generated (image_cost_usd present), and that no video has
already been completed for this segment (avoiding an accidental
double-spend) - then updates that same manifest.json in place with the
video result, exactly as segment 1A's original (pre-V1/V2-complication)
resume script did.
...
Safety, same pattern as every other real-call script in this repo:
  - Exactly 1 video-generation call. No image call, no retry, no other
    segment.
  - Hard cap set exactly equal to the computed video cost ($0.40) - zero
...
from scripts.run_full_video_segment_1b_test import (
    ASPECT_RATIO,
    DURATION_SECONDS,
    MANIFEST_PATH,
    OUTPUT_DIR,
...
MAX_SPEND_USD = 0.40  # video only - the image was already generated and paid for
POLL_INTERVAL_SECONDS = 3
MAX_WAIT_SECONDS = 300  # 5 minutes; see the timeout message below if this is hit

# LOCKED: the approved Segment 1B start frame. Must never be regenerated
# or pointed at a different file.
...
def main() -> None:
    ...
    print(f"\nVideo prompt (already-approved prompt, unchanged):\n  {VIDEO_PROMPT}")
    ...
    manifest["video_prompt"] = VIDEO_PROMPT
    ...
    print("  - ends with most of the floor joist system installed, still no floor decking/walls/roof/doors")
