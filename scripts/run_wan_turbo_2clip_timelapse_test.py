#!/usr/bin/env python3
"""WAN TURBO 2-CLIP TIMELAPSE TEST: ~10 seconds of accelerated construction
timelapse at the cheap $0.05/clip economics, as a controlled two-clip
sequence rather than switching to a different (more expensive) endpoint
to get one longer clip.

Clip 1: generated from the already-approved
`data/fal_mechanical_start_test/mechanical_start_frame.jpg` - the same
composition/mechanics source every prior gate in this project has used.
...
continuation of clip 1's actual visual state rather than an independently
imagined restart.

Both clips share the same builder/cabin/location/camera language; only
the construction-stage goal differs (clip 1: framing progress: lumber
carried/positioned, more studs and bracing appear; clip 2: continue
forward into a visibly later structural stage: upper framing, roof
framing, or cross-bracing - never a reset).

After both clips complete, they're concatenated into one review file via
`app/services/video_assembly.concatenate_videos` (ffmpeg's concat filter,
re-encodes - robust to minor stream-parameter differences between two
separately generated clips) purely so a human can review the ~10s
sequence as one file.

Safety, same pattern as every other real-call script in this repo:
  - Exactly 2 video calls, 81 frames each, 480p - hard-coded, no loop that
    could ever submit a 3rd. Zero image-generation calls (clip 2's source
    is a local ffmpeg frame extraction, not an API call).
  - Total cost ($0.10 - two flat, deterministic $0.05 charges) checked
...
MAX_SPEND_USD = 0.10  # two flat $0.05 charges - deterministic, zero margin needed
ASPECT_RATIO = "9:16"
RESOLUTION = "480p"
NUM_FRAMES = 81  # ~5.06s @ 16fps each - see WAN_TURBO's config for why this is pinned explicitly
...
OUTPUT_DIR = Path(__file__).resolve().parent.parent / "data" / "fal_wan_turbo_2clip_timelapse_test"
MANIFEST_PATH = OUTPUT_DIR / "manifest.json"

# Shared camera/attention/identity language, kept identical across both
# clips - only the construction-stage goal differs between them.
_SHARED_LANGUAGE = (
    "Vertical 9:16 accelerated construction timelapse, documentary/observational style. A "
    "rugged male builder in his 40s, wearing worn work clothes, works quickly on a timber cabin "
    "on a dramatic ocean-cliff site at golden hour. He is completely absorbed in the work and "
    "never looks toward the camera - his attention stays on the tools, materials, and structure. "
    "The camera stays in a mostly fixed side/three-quarter-rear observational position, as "
    "though someone is quietly filming him work from a slight distance; no orbiting, no dramatic "
    "camera moves, no centered hero framing. The ocean waves and wind add natural movement to "
    "the scene, and his clothing and loose materials move naturally too. The dramatic "
    "ocean-cliff landscape stays visible around the worksite. No posing, no presenter behavior, "
    "no commercial aesthetic - this should read as genuine, fast-paced construction timelapse "
    "footage."
)

CLIP_1_PROMPT = (
    _SHARED_LANGUAGE + "\n\nHe rapidly carries lumber, positions framing, and adds studs and "
    "bracing to the cabin structure - the framing becomes visibly denser and the cabin looks "
    "noticeably more complete by the end. Construction only moves forward; nothing already "
    "built disappears."
)

CLIP_2_PROMPT = (
    _SHARED_LANGUAGE + "\n\nBuilding on the framing already in place, he continues the "
    "construction forward into a visibly later stage - adding upper framing, roof framing, or "
    "cross-bracing to what's already built. The structure keeps advancing and becomes even more "
    "complete; nothing already built disappears, resets, or changes shape. This is a later, "
    "more advanced moment in the same continuous build, not a new or different structure."
)

SUCCESS_CRITERIA = [
    "Feels like accelerated construction across both clips",
    "Cabin visibly and cumulatively progresses from clip 1 into clip 2 (no reset)",
    "Builder never acknowledges the camera in either clip",
    "Location/environment remains recognizable throughout",
    "Major cabin geometry remains coherent (minor edge fuzziness/imperfect fingers/tool detail is fine)",
    "No catastrophic morphing or structure disappearing",
    "The ~10s combined sequence feels usable inside a longer final video",
]
...
def load_clip1_source_image() -> str:
    ...
    manifest.json - same reuse pattern as every prior script. Clip 2's
    source is NOT loaded this way - it's extracted locally from clip 1's
    own output after clip 1 completes (see main())."""
...
def generate_one_clip(provider: FalVideoProvider, label: str, prompt: str, source_image_path: str, video_path: Path):
    """Submit + poll + download exactly one clip. Returns (job_id, meta,
    actual_cost_usd). Calls fail() (exits) on any error - no retries."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Wan Turbo 2-clip timelapse test: 2 x $0.05 video generations, chained via last-frame extraction."
    )
    ...
    print("=" * 70)
    print("WAN TURBO 2-CLIP TIMELAPSE TEST - THIS WILL SPEND REAL MONEY")
    print("=" * 70)
    print(f"Clip 1 source (reused, $0 image cost): {clip1_source_path}")
    print(f"\nClip 1 prompt:\n  {CLIP_1_PROMPT}")
    ...
    manifest = {
        "created_at": _now(),
        "experiment": "wan_turbo_2clip_timelapse",
    ...
    print("\nReview the combined ~10s file against the success criteria:")
